...
# A Publisher Node Class for A star Algorithm
class Astar(Node):

    def __init__(self):

        super().__init__('tbot_astar')
        ...
        self.wheel_radius= 6.6

        self.wheel_dist = 28.7
        
        # Defining the rpm values and clearance
        ...
        offset = self.robot_radius + clearance
        ...
        # Defining the initial pose of the robot
        initial_x , initial_y , initial_theta = (0+500)*0.1 , (0+1000)*0.1 , 0
        ...
        while True:
            ...
            if map[int(approx_goal_x),int(approx_goal_y)] < 0:
                print("Please enter valid goal node value")
                continue
            break       
        ...
        def get_x_y_theta(rpm_right,rpm_left,x,y,theta):
            ...
            while count<20:
                dx=(self.wheel_radius/2)*(av_right+av_left)*math.cos(math.radians(new_theta))*0.05
                dy=(self.wheel_radius/2)*(av_right+av_left)*math.sin(math.radians(new_theta))*0.05
                dtheta=(self.wheel_radius/self.wheel_dist)*(av_right-av_left)*0.05
                new_x=new_x+dx
                new_y=new_y+dy
        ...
        while searching:
            ...
            #Checking if the goal node is within the distance of the present node
            if c2g <= 2:
                ...
                if not open_list:
                    ...
                    break
        # Extracting the RPM values (Action set rpm values) corresponding to the path as a list
        # Here each action has to be executed for 1 second       
        self.vel_set=final_node[2]
        self.index=0
        
        # Defining a Twist message object
        msg=Twist()
        
        while True:
            # After reaching the goal, the robot stops
            if self.index>=len(self.vel_set):
                while True: 
                    msg.linear.x = 0.0
                    msg.angular.z = 0.0
                    print("Linear Velocity: " , msg.linear.x , "     Angular Velocity: ",msg.angular.z)
                    self.move_publisher.publish(msg)
            else:
                # function to get angular velocity
                def get_angular_vel(rpm):
                    return (2*math.pi*rpm)/60
                # function to get linear and angular velocity of the turtlebot from the rpm values
                def get_linang(rpm_right,rpm_left):
                    av_right=get_angular_vel(rpm_right)
                    av_left=get_angular_vel(rpm_left)
                    lin_vel=(self.wheel_radius*0.5*(av_right+av_left))*0.1
                    ang_vel=(self.robot_radius*(1/self.wheel_dist)*(av_right-av_left))*0.1
                    return lin_vel,ang_vel
                (rpm_right,rpm_left)=self.vel_set[self.index]
                linear,angular = get_linang(rpm_right,rpm_left)
                msg.linear.x = linear*0.0965
                msg.angular.z = angular*3.14
                print("Linear Velocity: ",msg.linear.x," |  Angular Velocity: ",msg.angular.z)
                # Publishing the velocities
                self.move_publisher.publish(msg)
                self.index=self.index+1
                # sleeping for 1 seconf (Action set execution for 1 second)
                time.sleep(1.0)
...
def main(args=None):
    ...
    # Object of the node class
    control_astar = Astar()
    
    rclpy.spin(control_astar)

    control_astar.destroy_node()
